...
Channel.prototype.send = function (method, data, cb, chunkcb) {
  ...
  options.headers['user-agent'] = utils.getUserAgent();
    ...
    res.on('end', function () {
      ...
      parseContent(contentType, buf, function (err, content) {
        ...
        if (!err && res.statusCode && res.statusCode >= 400) {
          ...
          return cb && cb(ret, null, res);
        } else {
          return cb && cb(err, content, res);
        }
      });
    });
  }).on('error', function () {
...
function parseContent(contentType, buf, cb) {
  if (contentType) {
    if (contentType.indexOf('application/json') != -1) {
      ...
      // Remove BOM if present - SQL Azure management APIs include BOM in responses
...
Channel.open = function(managementEndpointUrl, managementCertificate) {
    var managementEndpoint = url.parse(managementEndpointUrl || account.managementEndpointUrl());
    var pem = managementCertificate || account.managementCertificate();
