...
describe('#context(dir, appname)', function() {
	...
	var ui = require(libpath + '/ui').empty;
	var fs = require('fs');
	var zeus = require(libpath + '/zeus');
	var sandbox;
	beforeEach(function() {
	...
	it('should throw if zeus context already exists', function(done) {
		...
		// Act/Assert
		zeus.context(ui, 'mydir', 'newapp', function(err, context) {
	...
	it('should return context with Zeusfile path if no file exists', function(done) {
		...
		// Act
		var context = zeus.context(ui, 'mydir', 'myApp', function(err, context) {
			// Assert
			assert.ifError(err);
