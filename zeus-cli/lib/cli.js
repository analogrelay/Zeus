// From azure-cli (with some tweaks)
...
function recordError(err) {
  if (err && err.stack) {
    try {
      fs.writeFileSync('zeus_error', (new Date()) + ':\n' +
          util.inspect(err) + '\n' + err.stack);
      (log.format().json ? log.error : log.info)('Error information has been recorded to zeus_error');
    } catch(err2) {
...
var debug = process.env.ZEUS_DEBUG === '1';
...
function enableNestedCommands(command) {
  ...
  command.on('*', function () {
    ...
    if (!command.categories[category]) {
      log.error('\'' + category + '\' is not an zeus command. See \'zeus help\'.');
    } else {
  ...
  function scan(scanPath) {
    ...
    results = results.filter(function (item) {
      try {
        ...
        return item.packageInfo && item.packageInfo.plugins && item.packageInfo.plugins['zeus-cli'];
      }
      catch (err) {
        return false;
      }
