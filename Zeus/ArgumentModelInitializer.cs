...
namespace Zeus
{
    // Modified from https://github.com/littlebits/args/blob/master/Args/ConventionBasedModelDefinitionInitializer.cs
    class ArgumentModelInitializer : IModelBindingDefinitionInitializer
    {
        public ArgumentModelInitializer()
        {
        }
        public void Initialize<TModel>(IModelBindingDefinition<TModel> init)
        {
            ...
            foreach (MemberInfo member in members)
            {
                ...
                if (argsTypeConverterAttribute != null)
                    memberBinding.TypeConverter = new ArgsTypeConverter(member.GetDeclaredType(), (IArgsTypeConverter)ArgsTypeResolver.Current.GetService(argsTypeConverterAttribute.ArgsTypeConverterType));
        ...
        {
            if (currentMember.Name.Length < iteration)
            ...
            if (subset.Count() == 0)
                throw new InvalidOperationException(String.Format(Properties.Resources.MemberNotFoundInAvailableMembers, currentMember.Name, currentMember.DeclaringType.FullName));
            else if (subset.Count() == 1)
                return attemptedValue;
            else
                return DeriveShortNameRecursive(subset, currentMember, iteration + 1);
        }
    }
}
